...
export async function POST(req: NextRequest) {
  try {
    ...
    await resend.emails.send({
      from: 'HelderCobra <info@helderdesign.nl>',
    ...
    console.log(err);
    //Whenever I get an unexpected error ===> send it to sentry (error login tool)
    return NextResponse.json(
      { message: 'Something went terrible wrong!', ok: false },
      { status: 500 },
    );
  }
}
